  ...
  return (
    <section className="py-24 md:py-32 container mx-auto px-6 md:px-12">
      <div className="grid lg:grid-cols-2 gap-16 items-center">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeInUp} className="relative rounded-2xl overflow-hidden glass-panel p-2 glow-effect">
          <img src="/images/workspace.webp" alt="Developer Workspace" className="w-full h-[400px] object-cover rounded-xl" />
        </motion.div>
        
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer}>
          <motion.h2 variants={fadeInUp} className="text-3xl md:text-5xl font-bold text-rn-text mb-6 leading-tight">
            Your Trusted <span className="text-rn-blue">Web Development</span> Partner
          </motion.h2>
          <motion.p variants={fadeInUp} className="text-lg text-rn-text-muted mb-8 leading-relaxed">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {[
              ...
              <motion.div variants={fadeInUp} key={idx} className="flex items-center gap-3">
                <div className="p-2 rounded-lg bg-rn-blue/10 text-rn-blue">
                  <item.icon size={18} />
                </div>
                <span className="text-rn-text-muted font-medium">{item.text}</span>
              </motion.div>
            ))}
          </div>
        </motion.div>
